...
{
    const rct_track_sequence** trackBlockArray = (ride_type_has_flag(td6->type, RIDE_TYPE_FLAG_HAS_TRACK)) ? TrackBlocks
                                                                                                          : FlatRideTrackBlocks;
    ...
    for (const auto& track : td6->track_elements)
    {
        ...
        switch (_trackDesignPlaceOperation)
        {
            case PTD_OPERATION_DRAW_OUTLINES:
                for (const rct_track_sequence* trackBlock = trackBlockArray[trackType]; trackBlock->index != 0xFF; trackBlock++)
                {
            ...
            case PTD_OPERATION_REMOVE_GHOST:
            {
                ...
                const rct_track_sequence* trackBlock = trackBlockArray[trackType];
