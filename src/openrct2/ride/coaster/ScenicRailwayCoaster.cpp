...
static void scenic_railway_rc_track_flat(
...
static void scenic_railway_rc_track_station(
...
/** rct2: 0x007783AC */
static void scenic_railway_rc_track_25_deg_up(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    if (tileElement->AsTrack()->HasChain())
...
/** rct2: 0x007783CC */
static void scenic_railway_rc_track_flat_to_25_deg_up(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    if (tileElement->AsTrack()->HasChain())
...
/** rct2: 0x007783FC */
static void scenic_railway_rc_track_25_deg_up_to_flat(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    if (tileElement->AsTrack()->HasChain())
...
/** rct2: 0x0077840C */
static void scenic_railway_rc_track_25_deg_down(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    scenic_railway_rc_track_25_deg_up(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

/** rct2: 0x0077842C */
static void scenic_railway_rc_track_flat_to_25_deg_down(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    scenic_railway_rc_track_25_deg_up_to_flat(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

/** rct2: 0x0077845C */
static void scenic_railway_rc_track_25_deg_down_to_flat(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    scenic_railway_rc_track_flat_to_25_deg_up(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

/** rct2: 0x0077846C */
static void scenic_railway_rc_track_left_quarter_turn_5(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
...
/** rct2: 0x0077847C */
static void scenic_railway_rc_track_right_quarter_turn_5(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    trackSequence = mapLeftQuarterTurn5TilesToRightQuarterTurn5Tiles[trackSequence];
    scenic_railway_rc_track_left_quarter_turn_5(session, rideIndex, trackSequence, (direction - 1) & 3, height, tileElement);
}

/** rct2: 0x0077848C */
static void scenic_railway_rc_track_s_bend_left(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (trackSequence)
...
/** rct2: 0x0077849C */
static void scenic_railway_rc_track_s_bend_right(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (trackSequence)
...
/** rct2: 0x007784DC */
static void scenic_railway_rc_track_left_quarter_turn_3(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (trackSequence)
...
/** rct2: 0x007784EC */
static void scenic_railway_rc_track_right_quarter_turn_3(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    trackSequence = mapLeftQuarterTurn3TilesToRightQuarterTurn3Tiles[trackSequence];
    scenic_railway_rc_track_left_quarter_turn_3(session, rideIndex, trackSequence, (direction - 1) & 3, height, tileElement);
}

/** rct2: 0x007784FC */
static void scenic_railway_rc_track_brakes(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (direction)
...
static void scenic_railway_rc_track_blockbrakes(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    if (tileElement->AsTrack()->HasChain())
...
/** rct2: 0x007785DC */
static void scenic_railway_rc_track_left_eighth_to_diag(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (trackSequence)
...
/** rct2: 0x007785EC */
static void scenic_railway_rc_track_right_eighth_to_diag(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (trackSequence)
...
/** rct2: 0x007785FC */
static void scenic_railway_rc_track_left_eighth_to_orthogonal(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    trackSequence = mapLeftEighthTurnToOrthogonal[trackSequence];
    scenic_railway_rc_track_right_eighth_to_diag(session, rideIndex, trackSequence, (direction + 2) & 3, height, tileElement);
}

/** rct2: 0x0077860C */
static void scenic_railway_rc_track_right_eighth_to_orthogonal(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    trackSequence = mapLeftEighthTurnToOrthogonal[trackSequence];
    scenic_railway_rc_track_left_eighth_to_diag(session, rideIndex, trackSequence, (direction + 3) & 3, height, tileElement);
}

/** rct2: 0x0077850C */
static void scenic_railway_rc_track_diag_flat(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (trackSequence)
...
/** rct2: 0x0077853C */
static void scenic_railway_rc_track_diag_25_deg_up(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (trackSequence)
...
/** rct2: 0x0077851C */
static void scenic_railway_rc_track_diag_flat_to_25_deg_up(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (trackSequence)
...
/** rct2: 0x0077852C */
static void scenic_railway_rc_track_diag_25_deg_up_to_flat(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (trackSequence)
...
/** rct2: 0x0077859C */
static void scenic_railway_rc_track_diag_25_deg_down(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (trackSequence)
...
/** rct2: 0x0077857C */
static void scenic_railway_rc_track_diag_flat_to_25_deg_down(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (trackSequence)
...
/** rct2: 0x0077858C */
static void scenic_railway_rc_track_diag_25_deg_down_to_flat(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (trackSequence)
...
static void scenic_railway_rc_track_60_deg_up(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (direction)
...
static void scenic_railway_rc_track_60_deg_down(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    scenic_railway_rc_track_60_deg_up(session, rideIndex, trackSequence, (direction + 2) % 4, height, tileElement);
}

static void scenic_railway_rc_track_25_deg_up_to_60_deg_up(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (direction)
...
static void scenic_railway_rc_track_60_deg_down_to_25_deg_down(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    scenic_railway_rc_track_25_deg_up_to_60_deg_up(session, rideIndex, trackSequence, (direction + 2) % 4, height, tileElement);
}

static void scenic_railway_rc_track_60_deg_up_to_25_deg_up(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (direction)
...
static void scenic_railway_rc_track_25_deg_down_to_60_deg_down(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    scenic_railway_rc_track_60_deg_up_to_25_deg_up(session, rideIndex, trackSequence, (direction + 2) % 4, height, tileElement);
}

static void scenic_railway_rc_track_diag_60_deg_up(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (trackSequence)
...
static void scenic_railway_rc_track_diag_60_deg_down(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    scenic_railway_rc_track_diag_60_deg_up(session, rideIndex, 3 - trackSequence, (direction + 2) % 4, height, tileElement);
}

static void scenic_railway_rc_track_diag_60_deg_up_to_25_deg_up(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (trackSequence)
...
static void scenic_railway_rc_track_diag_25_deg_down_to_60_deg_down(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    scenic_railway_rc_track_diag_60_deg_up_to_25_deg_up(
        session, rideIndex, 3 - trackSequence, (direction + 2) & 3, height, tileElement);
};

static void scenic_railway_rc_track_diag_25_deg_up_to_60_deg_up(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    switch (trackSequence)
...
static void scenic_railway_rc_track_diag_60_deg_down_to_25_deg_down(
    paint_session* session, ride_id_t rideIndex, uint8_t trackSequence, uint8_t direction, int32_t height,
    const TileElement* tileElement)
{
    scenic_railway_rc_track_diag_25_deg_up_to_60_deg_up(
        session, rideIndex, 3 - trackSequence, (direction + 2) & 3, height, tileElement);
};

TRACK_PAINT_FUNCTION get_track_paint_function_scenic_railway_rc(int32_t trackType)
{
    switch (trackType)
    {
        case TrackElemType::Flat:
            return scenic_railway_rc_track_flat;
        case TrackElemType::EndStation:
        case TrackElemType::BeginStation:
        case TrackElemType::MiddleStation:
            return scenic_railway_rc_track_station;
        case TrackElemType::Up25:
            return scenic_railway_rc_track_25_deg_up;
        case TrackElemType::FlatToUp25:
            return scenic_railway_rc_track_flat_to_25_deg_up;
        case TrackElemType::Up25ToFlat:
            return scenic_railway_rc_track_25_deg_up_to_flat;
        case TrackElemType::Down25:
            return scenic_railway_rc_track_25_deg_down;
        case TrackElemType::FlatToDown25:
            return scenic_railway_rc_track_flat_to_25_deg_down;
        case TrackElemType::Down25ToFlat:
            return scenic_railway_rc_track_25_deg_down_to_flat;
        case TrackElemType::LeftQuarterTurn5Tiles:
            return scenic_railway_rc_track_left_quarter_turn_5;
        case TrackElemType::RightQuarterTurn5Tiles:
            return scenic_railway_rc_track_right_quarter_turn_5;
        case TrackElemType::SBendLeft:
            return scenic_railway_rc_track_s_bend_left;
        case TrackElemType::SBendRight:
            return scenic_railway_rc_track_s_bend_right;
        case TrackElemType::LeftQuarterTurn3Tiles:
            return scenic_railway_rc_track_left_quarter_turn_3;
        case TrackElemType::RightQuarterTurn3Tiles:
            return scenic_railway_rc_track_right_quarter_turn_3;
        case TrackElemType::BlockBrakes:
            return scenic_railway_rc_track_blockbrakes;
        case TrackElemType::Brakes:
            return scenic_railway_rc_track_brakes;
        case TrackElemType::LeftEighthToDiag:
            return scenic_railway_rc_track_left_eighth_to_diag;
        case TrackElemType::RightEighthToDiag:
            return scenic_railway_rc_track_right_eighth_to_diag;
        case TrackElemType::LeftEighthToOrthogonal:
            return scenic_railway_rc_track_left_eighth_to_orthogonal;
        case TrackElemType::RightEighthToOrthogonal:
            return scenic_railway_rc_track_right_eighth_to_orthogonal;
        case TrackElemType::DiagFlat:
            return scenic_railway_rc_track_diag_flat;
        case TrackElemType::DiagUp25:
            return scenic_railway_rc_track_diag_25_deg_up;
        case TrackElemType::DiagFlatToUp25:
            return scenic_railway_rc_track_diag_flat_to_25_deg_up;
        case TrackElemType::DiagUp25ToFlat:
            return scenic_railway_rc_track_diag_25_deg_up_to_flat;
        case TrackElemType::DiagDown25:
            return scenic_railway_rc_track_diag_25_deg_down;
        case TrackElemType::DiagFlatToDown25:
            return scenic_railway_rc_track_diag_flat_to_25_deg_down;
        case TrackElemType::DiagDown25ToFlat:
            return scenic_railway_rc_track_diag_25_deg_down_to_flat;
        case TrackElemType::Down25ToDown60:
            return scenic_railway_rc_track_25_deg_down_to_60_deg_down;
        case TrackElemType::Down60ToDown25:
            return scenic_railway_rc_track_60_deg_down_to_25_deg_down;
        case TrackElemType::Up25ToUp60:
            return scenic_railway_rc_track_25_deg_up_to_60_deg_up;
        case TrackElemType::Up60ToUp25:
            return scenic_railway_rc_track_60_deg_up_to_25_deg_up;
        case TrackElemType::Up60:
            return scenic_railway_rc_track_60_deg_up;
        case TrackElemType::Down60:
            return scenic_railway_rc_track_60_deg_down;

        case TrackElemType::DiagUp60:
            return scenic_railway_rc_track_diag_60_deg_up;
        case TrackElemType::DiagDown60:
            return scenic_railway_rc_track_diag_60_deg_down;
        case TrackElemType::DiagUp60ToUp25:
            return scenic_railway_rc_track_diag_60_deg_up_to_25_deg_up;
        case TrackElemType::DiagDown25ToDown60:
            return scenic_railway_rc_track_diag_25_deg_down_to_60_deg_down;
        case TrackElemType::DiagUp25ToUp60:
            return scenic_railway_rc_track_diag_25_deg_up_to_60_deg_up;
        case TrackElemType::DiagDown60ToDown25:
            return scenic_railway_rc_track_diag_60_deg_down_to_25_deg_down;
    }
    return nullptr;
}
